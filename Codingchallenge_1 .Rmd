...
```{r}
colnames(zdf) <- c("zvec", "zlogic") #changing the names of column name within a dataframe
colnames(zdf)
```
...
```{r}
zdf$z2 <- (zdf$zvec)^2 #making a new column in the dataframe
zdf
```
...
```{r}
subset(zdf, (z2 > 10) & (z2 < 100) ) #subsetting using subset function
```

```{r}
zdf$z2[((zdf$z2 > 10) & (zdf$z2 <  100))] #subsetting without using subset function
```

o	Subset the zdf dataframe to only include the values on row 26
```{r}
zdf[26,] #subsetting to only include a particular row
```

o	Subset the zdf dataframe to only include the values in the column zsquared in the 180th row.
```{r}
zdf$z2[180] #subsetting to include only a value in a row within a column
```
